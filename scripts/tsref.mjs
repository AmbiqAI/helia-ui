...
/*
 * Turns a TypeDoc JSON dump into a published TypeScript API reference: MDX
 * pages that compose the package's `Ref*` parts, the model as JSON, and the
 * text artifacts an agent reads instead of the pages.
 *
 *   npx typedoc --json build/api.json src/index.ts
 *   helia-ui-tsref --json build/api.json --out src/content/docs/reference/api \
 *                  --public public --base /helia-sdk/ \
 *                  --sidebar src/generated/api-sidebar.json
 *
 * Two steps, not one. `tsref-extract.mjs` maps the reflections onto the
 * reference model and is the only file here that knows TypeScript;
 * `reference-render.mjs` turns a model into pages and artifacts and is the
 * same renderer the Python and C references go through. The model is the
 * source of truth and the pages are a view of it, which is why the JSON is
 * published rather than kept in a build directory.
 *
 ...
 * installed but the package itself. TypeDoc produced the input somewhere else
 * and is not needed to publish it.
 */
 ...
 */
const MANIFEST = '.tsref-manifest.json';
const MANIFEST_VERSION = 1;

const ALL_DEFAULTS = { ...DEFAULTS, ...RENDER_DEFAULTS };

const USAGE = `helia-ui-tsref --json <file> --out <dir> --base <site base path>

  --json <file>         TypeDoc JSON dump (schema ${TYPEDOC_SCHEMA_VERSION}), from typedoc --json
  ...
  --name <library>      the library the reference documents (default the package name)
...
Rendering options, shared with helia-ui-pyref and helia-ui-doxyref:
...
const jsonFile = resolve(values.json);
let project;
try {
  project = await readTypedocJson(jsonFile);
} catch (error) {
  if (error instanceof TsrefSchemaError) die(error.message);
  throw error;
}
...
if (values.check) {
  ...
  if (drift.length > 0) {
    console.error(`${show(outDir)} has drifted from ${show(jsonFile)}:\n`);
    for (const line of drift.slice(0, 50)) console.error(`  ${line}`);
    if (drift.length > 50) console.error(`  ... and ${drift.length - 50} more`);
    console.error('\nRe-run helia-ui-tsref without --check and commit.');
    process.exit(1);
  }
  console.log(
    `tsref: ${pages.length} pages and ${artifacts.length} artifacts up to date.`,
  );
} else {
  ...
  console.log(
    `tsref: ${pages.length} pages written to ${show(outDir)}, ${artifacts.length} artifacts to ${show(publicDir)}.`,
  );
}

if (warnings.length > 0) {
  const shown = values.quiet ? [] : warnings;
  for (const warning of shown) console.warn(`tsref warning: ${warning}`);
  console.warn(`tsref: ${warnings.length} warnings.`);
}
