...
colours come from the same tokens as the rest of the page, so a diagram follows
...
<Callout title="The build needs a headless browser" tone="important">
  ...
  is the behaviour to want: the alternative is a page that deploys with empty
  figures.
</Callout>
...
Expressive Code, which tokenises the source into hundreds of spans and leaves
