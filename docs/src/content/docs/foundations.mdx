...
[colour reference](/helia-ui/foundations/tokens/) carries the values themselves,
...
the package writes a colour literal outside the token files; everything else
...
Every colour token, its value in each theme, and the contrast each ink reaches
on the canvas and on the card are on the
[colour reference](/helia-ui/foundations/tokens/). The accents are shown
...
One ramp of thirteen sizes, five weights, three leadings, and four tracking
...
face rather than to the size, so a component that sets a negative
letter-spacing of its own is holding a value the ramp should own.
...
Advisory colour comes in pairs. The tone is the fill an icon, a rim, or an edge
...
status colour on a word takes the ink, never the fill.
...
[colour reference](/helia-ui/foundations/tokens/#ink-on-surface), and
